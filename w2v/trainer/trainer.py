...
class NegSamplingTrainer():
    def __init__(self, dataloader, config):
        ...
        self.target_emb = nn.Embedding(len(dataloader.dataset), config.embed_dim).float().to(config.device)
        self.context_emb = nn.Embedding(len(dataloader.dataset), config.embed_dim).float().to(config.device)
        ...
        self.optimizer = torch.optim.SGD(list(self.target_emb.parameters()) + list(self.context_emb.parameters()), lr=5e-3, momentum=0.9)
    ...
    def train():
        for epoch in range(self.config.epoch):
            ...
            for i, sample in enumerate(self.dataloader):
                ...
                pred_neg = self.sigmoid(self.similar(neg, target.unsqueeze(1).expand_as(neg)))
